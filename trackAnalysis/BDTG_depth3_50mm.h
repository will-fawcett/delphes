...
class ReadBDTG50 : public IClassifierReader {

 public:

   // constructor
   ReadBDTG50( std::vector<std::string>& theInputVars ) 
      : IClassifierReader(),
        fClassName( "ReadBDTG50" ),
        fNvars( 9 ),
        fIsNormalised( false )
   {      
...
double ReadBDTG50::GetMvaValue__( const std::vector<double>& inputValues ) const
{
   double myMVA = 0;
...
// Clean up
inline void ReadBDTG50::Clear() 
{
   for (unsigned int itree=0; itree<fForest.size(); itree++) { 
      delete fForest[itree]; 
   }
}
   inline double ReadBDTG50::GetMvaValue( const std::vector<double>& inputValues ) const
   {
