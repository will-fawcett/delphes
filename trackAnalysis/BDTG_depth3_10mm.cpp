...
class ReadBDTG10 : public IClassifierReader {

 public:

   // constructor
   ReadBDTG10( std::vector<std::string>& theInputVars ) 
      : IClassifierReader(),
        fClassName( "ReadBDTG10" ),
        fNvars( 9 ),
        fIsNormalised( false )
   {      
...
double ReadBDTG10::GetMvaValue__( const std::vector<double>& inputValues ) const
{
   double myMVA = 0;
...
// Clean up
inline void ReadBDTG10::Clear() 
{
   for (unsigned int itree=0; itree<fForest.size(); itree++) { 
      delete fForest[itree]; 
   }
}
   inline double ReadBDTG10::GetMvaValue( const std::vector<double>& inputValues ) const
   {
